...
DEF_TEST(SkParagraph_RainbowParagraph, reporter) {
    ...
    text_style2.setFontStyle(SkFontStyle(SkFontStyle::kSemiBold_Weight, SkFontStyle::kNormal_Width,
                                         SkFontStyle::kUpright_Slant));
    text_style2.setLetterSpacing(10);
    text_style2.setDecorationColor(SK_ColorBLACK);
    text_style2.setDecoration((SkTextDecoration)(SkTextDecoration::kUnderline |
    ...
    text_style4.setDecoration((SkTextDecoration)(SkTextDecoration::kUnderline |
                                                 SkTextDecoration::kOverline |
                                                 SkTextDecoration::kLineThrough));
...
DEF_TEST(SkParagraph_ChineseParagraph, reporter) {
    ...
    auto decoration =
            (SkTextDecoration)(SkTextDecoration::kUnderline | SkTextDecoration::kOverline |
                               SkTextDecoration::kLineThrough);
...
DEF_TEST(SkParagraph_GetGlyphPositionAtCoordinateParagraph, reporter) {
    ...
    REPORTER_ASSERT(reporter,
                    paragraph->getGlyphPositionAtCoordinate(100000, 20).position == 17);  // !!! 18
    REPORTER_ASSERT(reporter, paragraph->getGlyphPositionAtCoordinate(450, 20).position == 16);
    REPORTER_ASSERT(reporter,
                    paragraph->getGlyphPositionAtCoordinate(100000, 90).position == 35);  // !!! 36
    ...
    REPORTER_ASSERT(reporter,
                    paragraph->getGlyphPositionAtCoordinate(10000, 180).position == 71);  // !!! 72
...
DEF_TEST(SkParagraph_GetRectsForRangeIncludeLineSpacingMiddle, reporter) {
    ...
    }
    {
        ...
        REPORTER_ASSERT(reporter, result.size() == 1);
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.left(), 0, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.top(), 8.60, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 16.90, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 93.60, epsilon));
    }
    {
        auto result = paragraph->getRectsForRange(2, 8, heightStyle, widthStyle);
        REPORTER_ASSERT(reporter, result.size() == 1);
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.left(), 66.90, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.top(), 8.60, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 264.10, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 93.60, epsilon));
    }
    {
        auto result = paragraph->getRectsForRange(8, 21, heightStyle, widthStyle);
        REPORTER_ASSERT(reporter, result.size() == 2);
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.left(), 264.10, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.top(), 8.60, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 528.20, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 93.60, epsilon));
    }
    {
        auto result = paragraph->getRectsForRange(30, 150, heightStyle, widthStyle);
        REPORTER_ASSERT(reporter, result.size() == 9);
        SkScalar offsetY = 104.60;
        for (auto& box : result) {
            if (&box != &result.back()) {
                REPORTER_ASSERT(reporter, SkScalarNearlyEqual(box.rect.right(), 528.20, epsilon));
            }
            if (&box != &result.front()) {
    ...
    }
    {
        auto result = paragraph->getRectsForRange(19, 22, heightStyle, widthStyle);
        REPORTER_ASSERT(reporter, result.size() == 1);
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.left(), 97.20, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.top(), 104.60, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.right(), 197.20, epsilon));
        REPORTER_ASSERT(reporter, SkScalarNearlyEqual(result[0].rect.bottom(), 189.60, epsilon));
    }
    {
        auto result = paragraph->getRectsForRange(21, 21, heightStyle, widthStyle);
        REPORTER_ASSERT(reporter, result.empty());
...
DEF_TEST(SkParagraph_KernScaleParagraph, reporter) {
    ...
    REPORTER_ASSERT(reporter,
                    SkScalarNearlyEqual(impl->runs()[0].calculateHeight(), 39.046, epsilon));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->runs()[1].advance().fX, 214.85, epsilon));
    REPORTER_ASSERT(reporter,
                    SkScalarNearlyEqual(impl->runs()[1].calculateHeight(), 5.466, epsilon));
}
...
DEF_TEST(SkParagraph_NewlineParagraph, reporter) {
    ...
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[2].width(), 593.49, epsilon));
    ...
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->lines()[6].width(), 137.16, epsilon));
        ...
        line.scanStyles(
                ...
                [&index, reporter](SkTextStyle style, SkSpan<const char> text) {
                    switch (index) {
                        ...
                        case 1:
                            REPORTER_ASSERT(reporter,
                                            style.getDecoration() == SkTextDecoration::kUnderline);
                            break;
                        default:
                            REPORTER_ASSERT(reporter, false);
                            break;
...
DEF_TEST(SkParagraph_BaselineParagraph, reporter) {
    ...
    SkScalar epsilon = 0.01;
    REPORTER_ASSERT(reporter,
                    SkScalarNearlyEqual(paragraph->getIdeographicBaseline(), 79.035, epsilon));
    REPORTER_ASSERT(reporter,
                    SkScalarNearlyEqual(paragraph->getAlphabeticBaseline(), 63.305, epsilon));
}
...
DEF_TEST(SkParagraph_FontFallbackParagraph, reporter) {
    ...
    SkTextStyle text_style;
    text_style.setFontFamilies({"Not a real font", "Also a fake font", "So fake it is obvious",
                                "Next one should be a real font...", "Roboto",
                                "another fake one in between", "Homemade Apple"});
    text_style.setColor(SK_ColorBLACK);
    builder.pushStyle(text_style);
    builder.addText(text1);

    text_style.setFontFamilies({"Not a real font", "Also a fake font", "So fake it is obvious",
                                "Homemade Apple", "Next one should be a real font...", "Roboto",
                                "another fake one in between", "Noto Sans CJK JP",
                                "Source Han Serif CN"});
    builder.pushStyle(text_style);
    builder.addText(text2);
    ...
    text_style.setFontFamilies({"Not a real font", "Also a fake font", "So fake it is obvious",
                                "Homemade Apple", "Next one should be a real font...", "Roboto",
                                "another fake one in between", "Source Han Serif CN",
                                "Noto Sans CJK JP"});
    builder.pushStyle(text_style);
    builder.addText(text3);
    ...
    // Font resolution in Skia produces 6 runs because 2 parts of "Roboto 字典 " have different script
    // (Minikin merges the first 2 into one because of unresolved)
    // [Apple + Unresolved ] [Apple + Noto] [Apple + Han]
    REPORTER_ASSERT(reporter, impl->runs().size() == 6);

    SkScalar epsilon = 0.01;
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->runs()[0].advance().fX, 48.46, epsilon));
    REPORTER_ASSERT(reporter, SkScalarNearlyEqual(impl->runs()[1].advance().fX, 15.90, epsilon));
