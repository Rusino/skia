...
void SkParagraphImpl::paint(SkCanvas* canvas, double x, double y) {
  ...
  if (nullptr == fPicture) {
    ...
    if (!fRuns.empty()) {
      SkFontSizes maxSizes = fRuns.begin()->sizes();
      for (auto& line : fTextWrapper.getLines()) {
        ...
        this->iterateThroughStyles(line, SkStyleType::Background,
                                   [this, textCanvas, line](SkSpan<const char> text, SkTextStyle style, SkRun* ellipsis) {
                                     this->paintBackground(textCanvas, line, text, style, ellipsis);
                                     return true;
                                   });

        this->iterateThroughStyles(line, SkStyleType::Shadow,
                                   [this, textCanvas, line](SkSpan<const char> text, SkTextStyle style, SkRun* ellipsis) {
                                     this->paintShadow(textCanvas, line, text, style, ellipsis);
                                     return true;
                                   });

        this->iterateThroughStyles(line, SkStyleType::Foreground,
                                   [this, textCanvas, line](SkSpan<const char> text, SkTextStyle style, SkRun* ellipsis) {
                                     this->paintText(textCanvas, line, text, style, ellipsis);
                                     return true;
                                   });

        this->iterateThroughStyles(line, SkStyleType::Decorations,
                                   [this, textCanvas, line](SkSpan<const char> text, SkTextStyle style, SkRun* ellipsis) {
                                     this->paintDecorations(textCanvas, line, text, style, ellipsis);
                                     return true;
                                   });
        textCanvas->restore();
      }
...
void SkParagraphImpl::paintText(
  ...
  this->iterateThroughRuns(line, text, ellipsis,
...
void SkParagraphImpl::buildClusterTable() {
  ...
  for (auto& run : fRuns) {
    ...
    for (size_t pos = 0; pos <= run.size(); ++pos) {
      ...
      auto next = pos == run.size() ? run.fUtf8Range.size() : run.cluster(pos);
      ...
      data.fEnd = pos;
      // TODO: Better methods
      data.fText = SkSpan<const char>(fUtf8.begin() + run.fUtf8Range.begin() + cluster, next - cluster);
      data.fWidth = width;
      data.fHeight = run.calculateHeight();
      fClusters.emplace_back(data);
      fIndexes.set(fUtf8.begin() + run.fUtf8Range.begin() + cluster, fClusters.size() - 1);
...
void SkParagraphImpl::shapeTextIntoEndlessLine(SkSpan<const char> text, SkSpan<SkBlock> styles) {
 ...
 class MultipleFontRunIterator final : public SkShaper::FontRunIterator {
  ...
  class ShapeHandler final : public SkShaper::RunHandler {
   ...
   private:
    ...
    Buffer runBuffer(const RunInfo& info) override {
      ...
      auto& run = fParagraph->fRuns.emplace_back(info);
  ...
  auto bidi = shaper->MakeBidiRunIterator(fUtf8.begin(), fUtf8.size(), true);
  auto script = shaper->MakeScriptRunIterator(fUtf8.begin(), fUtf8.size());
  auto lang = shaper->MakeLanguageRunIterator(fUtf8.begin(), fUtf8.size());
...
void SkParagraphImpl::justifyLine(SkLine& line, SkScalar maxWidth) {
  ...
  for (auto& word : line.fWords) {
    ...
    this->iterateThroughRuns(line, word.text(), nullptr,
...
void SkParagraphImpl::iterateThroughRuns(
  ...
  for (auto cluster = start; cluster <= end; ++cluster) {
    ...
    if (cluster == start) {
      ...
      clip.fLeft += cluster->sizeToChar(line.text().begin());
    }
    if (cluster == end) {
      clip.fRight += cluster->sizeFromChar(line.text().end() - 1);
    } else {
