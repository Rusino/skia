                        ...
                        SkScalar width) const
{
    ...
    if (false) {
      return fImpl->shapeCorrect(handler, utf8, utf8Bytes, point, width, runSegmenter, bidi, script, font);
    } else {
                                     ...
                                     const FontRunIterator* font) const
{
    ...
    while (runSegmenter.advanceRuns()) {  // For each item
        ...
        ShapedRun model = shape(utf8, utf8Bytes,
                                utf8Start, utf8End,
                                bidi, script, font);
        int modelOffset = 0;
        ...
        while (utf8Start < utf8End) {  // While there are still code points left in this item
            ...
            if (width < line.fAdvance.fX + best.fAdvance.fX && !line.runs.empty()) {
                ...
                if (!best.fGlyphs) {
                    ...
                    modelText[utf8End - best.fUtf8End].glyphLen = model.fNumGlyphs;
                    modelText[utf8End - best.fUtf8End].advance = model.fAdvance;
                    modelTextOffset = 0;
