...
7. **The Interactive Hand-off & Verification Call-to-Action Protocol**:
   - По завершении реализации, прохождения тестов и локального коммита агент обязан не просто констатировать факт, а явно передать управление Overgod:
     1. Предоставить точную готовую команду для запуска собранного приложения/артефакта.
     2. Сформулировать краткий пошаговый сценарий ручной проверки (User Verification Script), фокусируясь на граничных условиях исправленных дефектов.
     3. Явно запросить результаты проверки и обозначить следующий приоритетный шаг бэклога.
